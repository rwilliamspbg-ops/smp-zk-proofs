...
use crate::pq_compatibility::lattice_backend::LatticeProof;
use crate::pq_compatibility::{PqcBackendType, PqcConfig, PqcError, PqcResult};
...
pub enum PqcProofEnvelope {
    ...
    Hybrid(Box<HybridProofEnvelope>),
}
...
pub struct HybridProofEnvelope {
    ...
    /// PQC proof component
    pub pqc_proof: Box<PqcProofEnvelope>,
...
pub fn generate_pqc_location_proof(
    ...
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<PqcProofEnvelope> {
    match config.backend_type {
        PqcBackendType::Lattice => {
            let lattice_proof =
                crate::pq_compatibility::lattice_backend::generate_pqc_location_proof(
                    location_data,
                    public_inputs,
                    config,
                )?;
...
pub fn generate_pqc_training_proof(
    ...
    match config.backend_type {
        PqcBackendType::Lattice => {
            let lattice_proof =
                crate::pq_compatibility::lattice_backend::generate_pqc_training_proof(
                    training_data,
                    public_inputs,
                    config,
                )?;
...
fn generate_hash_location_proof(
    location_data: &[u8],
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<HashProof> {
    ...
    let merkle_root: Vec<u8> = Into::<[u8; 32]>::into(hasher.finalize()).to_vec();

    // Generate authentication path (simplified - single leaf)
    let auth_path = vec![merkle_root.clone()];

    // Generate challenge (= response for this simplified hash-based scheme)
    let mut chasher = Sha256::new();
    chasher.update(&merkle_root);
    chasher.update(public_inputs);
    let response: Vec<u8> = Into::<[u8; 32]>::into(chasher.finalize()).to_vec();
    ...
    Ok(HashProof {
        ...
        response,
        public_inputs_hash,
        metadata: ProofMetadata::new("hash-sha256", config.security_level_bits),
    })
}

fn verify_hash_location_proof(
    proof: &HashProof,
    public_inputs: &[u8],
    _config: &PqcConfig,
) -> PqcResult<bool> {
    ...
    chasher.update(public_inputs);
    let expected_challenge: Vec<u8> = Into::<[u8; 32]>::into(chasher.finalize()).to_vec();

    if expected_challenge != proof.response {
        return Ok(false);
    }
    ...
    Ok(expected_pi_hash == proof.public_inputs_hash)
}

fn generate_hash_training_proof(
    training_data: &[u8],
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<HashProof> {
...
fn generate_code_location_proof(
    _location_data: &[u8],
    _public_inputs: &[u8],
    _config: &PqcConfig,
) -> PqcResult<CodeProof> {
...
fn generate_code_training_proof(
    _training_data: &[u8],
    _public_inputs: &[u8],
    _config: &PqcConfig,
) -> PqcResult<CodeProof> {
...
fn generate_hybrid_location_proof(
    location_data: &[u8],
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<PqcProofEnvelope> {
    ...
    let classical_proof: Vec<u8> = Into::<[u8; 32]>::into(classical_hasher.finalize()).to_vec();
    ...
    let mut linker = Sha256::new();
    linker.update(&classical_proof);
    let LatticeProof { commitment, .. } = &pqc_proof;
    linker.update(&commitment.commitment);
    linker.update(public_inputs);
    let linkage_proof: Vec<u8> = Into::<[u8; 32]>::into(linker.finalize()).to_vec();

    Ok(PqcProofEnvelope::Hybrid(Box::new(HybridProofEnvelope {
        classical_proof,
        pqc_proof: Box::new(PqcProofEnvelope::Lattice(pqc_proof)),
        linkage_proof,
        metadata: ProofMetadata::new("hybrid-classical-pqc", config.security_level_bits),
    })))
}

fn verify_hybrid_location_proof(
    proof: &HybridProofEnvelope,
    public_inputs: &[u8],
    _config: &PqcConfig,
) -> PqcResult<bool> {
    use sha2::{Digest, Sha256};
    ...
    // Verify PQC proof
    let pqc_valid = match proof.pqc_proof.as_ref() {
        PqcProofEnvelope::Lattice(lattice_proof) => {
            crate::pq_compatibility::lattice_backend::verify_pqc_location_proof(
                lattice_proof,
                public_inputs,
    ...
    if let PqcProofEnvelope::Lattice(lp) = proof.pqc_proof.as_ref() {
        linker.update(&lp.commitment.commitment);
    }
    linker.update(public_inputs);
    let expected_linkage: Vec<u8> = Into::<[u8; 32]>::into(linker.finalize()).to_vec();

    Ok(expected_linkage == proof.linkage_proof)
}

fn generate_hybrid_training_proof(
    training_data: &[u8],
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<PqcProofEnvelope> {
...
fn verify_hybrid_training_proof(
    proof: &HybridProofEnvelope,
    public_inputs: &[u8],
    config: &PqcConfig,
) -> PqcResult<bool> {
